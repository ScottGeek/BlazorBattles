...
namespace BlazorBattles.Server.Controllers
{
    ...
    public class BattleController : ControllerBase
    {
        ...
        {
            int randomAttackerIndex = new Random().Next(attackerArmy.Count);  //Pick an amry at random
            int randomOpponentIndex = new Random().Next(opponentArmy.Count);  //Pick an amry at random
        ...
        private async Task FinishFight(User attacker, User opponent, BattleResult result,
            int attackerDamageSum, int opponetDamageSum)
        {
            result.AttackerDamageSum = attackerDamageSum;
            result.OpponentDamageSum = opponetDamageSum;
            ...
            if (result.IsVictory)
            {
                attacker.Victories++;
                opponent.Defeats++;
                attacker.Bananas += opponetDamageSum;
            ...
            else
            {
                attacker.Defeats++;
                opponent.Victories++;
                attacker.Bananas += opponetDamageSum * 10;
